...
class CrossrefExportDeployment extends PKPImportExportDeployment {
	...
	function createHeadNode($documentNode) {
		...
		$depositorNode->appendChild($node = $documentNode->createElementNS($this->getNamespace(), 'email_address', $this->xmlEscape('dgdicd.comunicacao@abcd.usp.br')));
	...
	function createContributorsNode($documentNode, $authors, $locale, $isChapter = false) {
		...
		foreach ($authors as $author) {
			...
			// Adiciona o nó da afiliação após o nó person_name
			if ($author->getLocalizedAffiliation()) {
				$organizationNode = $documentNode->createElement('organization', $author->getLocalizedAffiliation());
				$organizationNode->setAttribute('sequence', $isFirst ? 'first' : 'additional');
				$organizationNode->setAttribute('contributor_role', 'author');
